...
function Scroll() {
  ...
  return (
    <>
      {showTopArrow && (
        <div
          className="fixed bottom-10 right-0 bg-blue-600 text-white p-3 rounded-full cursor-pointer z-40"
          onClick={scrollToTop}
        >
          <FaArrowUp size={14} />
        </div>
      ...
      <div className="fixed right-0 top-2/3 flex flex-col items-center space-y-4 z-50 bg-[#2F6690] rounded-md">
        <div
          className="p-2 rounded-full shadow-md hover:bg-gray-200 cursor-pointer"
          onClick={() => handleIconClick("phone")}
        >
          <FaPhone size={40} className="text-blue-400" />
        </div>
        <div
          className="p-2 rounded-full shadow-md hover:bg-gray-200 cursor-pointer"
